...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i += 1)
    {
        ...
        {
            int candidate_pref_vot = preferences[i][j];

            if (!candidates[candidate_pref_vot].eliminated)
            {
                candidates[candidate_pref_vot].votes += 1;
                break;
            }
        }
    }
...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i += 1)
    {
        if (votes_to_win < candidates[i].votes)
        {
            printf("%s\n", candidates[i].name);
            return true;
        }
